...
st.set_page_config(
    page_title="Sio Basin",
    page_icon="🏞️",
    layout="wide"
)

# Data folder path
DATA_DIR = Path(__file__).parent.parent / "data" / "sio_basin"
...
st.title("Sio Basin")
st.markdown("*Trans-boundary river basin spanning Kenya and Uganda*")
...
with tab1:
    ...
    if html_content:
        ...
        st.warning(f"3D visualization not found. Please add `3d_visualization.html` to `data/sio_basin/`")
...
with tab2:
    ...
    if html_content:
        ...
        st.warning(f"Split map not found. Please add `split_map.html` to `data/sio_basin/`")
        st.info("Expected path: " + str(html_file))
