...
st.set_page_config(
    page_title="Lower Mara Basin",
    page_icon="🏞️",
    layout="wide"
)

# Data folder path
DATA_DIR = Path(__file__).parent.parent / "data" / "lower_mara_basin"
...
st.title("Lower Mara Basin")
st.markdown("*Lower section of the Mara River basin*")
st.markdown('#### Basin Area : 3826 sq km')
...
with tab1:
    ...
    if html_content:
        ...
        st.warning(f"3D visualization not found. Please add `3d_visualization.html` to `data/lower_mara_basin/`")
...
with tab2:
    ...
    if html_content:
        ...
        st.warning(f"Split map not found. Please add `split_map.html` to `data/lower_mara_basin/`")
        st.info("Expected path: " + str(html_file))
